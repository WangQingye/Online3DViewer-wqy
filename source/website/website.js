...
export class Website
{
    ...
    InitToolbar ()
    {
        ...
        // AddButton (this.toolbar, 'fit', '将模型适配到窗口', ['only_on_model'], () => {
        //     this.FitModelToWindow (false);
        // });
        AddButton (this.toolbar, 'up_y', 'Y轴朝上', ['only_on_model'], () => {
            this.viewer.SetUpVector (Direction.Y, true);
        });
        AddButton (this.toolbar, 'up_z', 'Z轴朝上', ['only_on_model'], () => {
            this.viewer.SetUpVector (Direction.Z, true);
        });
        AddButton (this.toolbar, 'flip', '翻转', ['only_on_model'], () => {
            this.viewer.FlipUpVector ();
        });
        AddSeparator (this.toolbar, ['only_on_model']);
        AddRadioButton (this.toolbar, ['fix_up_on', 'fix_up_off'], ['固定视角', '自由视角'], 0, ['only_on_model'], (buttonIndex) => {
            if (buttonIndex === 0) {
